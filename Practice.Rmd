...
```{r}
hierarchical_quarter_model_new <- stan_glmer(home_team_win ~ score_differential.x + 
                                        score_differential.y +
...
```{r}
pp_check(hierarchical_quarter_model_new)
```

```{r}
classification_summary(hierarchical_quarter_model_new,quarter_score_diff)
```
...
scoring_play_data<- data_new_2  %>%
  ...
  filter(field_goal_result_ == 1 |
           ...
           safety == 1) %>%
  select(home_team, away_team, field_goal_result_, extra_point_result_, touchdown, two_point_conv_result_, safety, game_id, home_score, away_score, home_team_win, score_differential_post, game_seconds_remaining)


down_sample_scoring <- scoring_play_data %>%
  filter(game_id == "2020_16_CHI_JAX" | game_id == "2020_01_CLE_BAL"| game_id == "2020_08_LAC_DEN" | game_id == "2020_07_BUF_NYJ" | game_id == "2020_16_PHI_DAL" | game_id == "2020_14_MIN_TB" | game_id == "2020_01_CHI_DET" | game_id == "2020_03_DET_ARI" | game_id == "2020_16_CIN_HOU" | game_id == "	
2020_06_DEN_NE" | game_id == "2020_07_PIT_TEN" | game_id == "2020_03_WAS_CLE" | game_id == "2020_07_SEA_ARI
" | game_id == "2020_03_SF_NYG" | game_id == "2020_08_NE_BUF" | game_id == "2020_09_NO_TB" | game_id == "2020_06_WAS_NYG" | game_id == "2020_12_CLE_JAX" | game_id == "2020_10_BUF_ARI" | game_id == "2020_04_NO_DET")
...
```{r}
scoring_play_model <- stan_glmer(home_team_win ~ score_differential_post + game_seconds_remaining + (1|game_id), 
...
```{r}
game_pred_2 <- posterior_predict(scoring_play_model, data.frame(score_differential_post = -5, game_seconds_remaining = 1200, game_id = "2021_12_DAL_LV"))

mean(game_pred_2)

game_pred_3 <- posterior_predict(scoring_play_model, data.frame(score_differential_post = 50, game_seconds_remaining = 40, game_id = "2021_6_PIT_SEA"))
