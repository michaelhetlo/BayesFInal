...
first_q_diff <- data_new %>%
  ...
  mutate(score_differential_1 = total_home_score- total_away_score)%>%
  select(game_id, home_team, away_team, score_differential)
...
```{r}
hierarchical_half_model <- stan_glm(home_team_win ~ score_differential.x + 
...
classification_summary(model = hierarchical_half_model, data = quarter_score_diff)
```
