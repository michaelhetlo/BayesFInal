...
```{r}
classification_summary(hierarchical_quarter_model_new,quarter_score_diff,cutoff = .5)
```

```{r}
model_sum <- tidy(hierarchical_quarter_model_new, conf.int = TRUE, conf.level = .95)
model_sum
```

```{r}
exp(model_sum$estimate)/(1+exp(model_sum$estimate))
```
Our mean population states that at the start of the beginning of the game we predict that the home team will win in regulation 41% of the time. 

Score.x is the increase in win probability for score differential 
...
```{r}
game_pred <- posterior_predict(hierarchical_quarter_model_new, data.frame(score_differential.x = -16,score_differential.y = -8, score_differential.x.x = 8, game_id = "2021_12_DAL_LV"))
```
