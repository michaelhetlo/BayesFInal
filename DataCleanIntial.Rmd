...
```{r}
pbp %>%
  mutate(yardstoendzone = ifelse(YardLineDirection == 'OWN', YardLineFixed + 50, YardLineFixed)) %>%
  group_by(yardstoendzone) %>%
  summarise(n())
```
...
```{r}
pbp %>%
  group_by(YardLineFixed, YardLineDirection, YardLine) %>%
  summarise(n())
```
