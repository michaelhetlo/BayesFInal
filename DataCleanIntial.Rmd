...
loadedpbp %>%
  ...
  group_by(offteam) %>%
  summarise(total = sum(passing_yards, na.rm = TRUE)) %>%
  arrange(desc(total))
```
...
loadedpbp %>%
  filter(season == 2018, 
         week <= 17) %>%
  group_by(offteam) %>%
  summarise(total.yards = sum(yards_gained, na.rm = TRUE), pass.yards = sum(passing_yards, na.rm = TRUE), rushing_yards = sum(rushing_yards, na.rm = TRUE), ints.thrown = sum(interception, na.rm = TRUE), fumble.lost = sum(fumble_lost, na.rm = TRUE), turnovers = ints + fumble.lost)
```
